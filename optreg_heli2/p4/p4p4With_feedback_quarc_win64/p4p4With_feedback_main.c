...
typedef RT_MODEL_p4p4With_feedback_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * p4p4With_feedback(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(p4p4With_feedback_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(p4p4With_feedback_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: p4p4With_feedback -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(p4p4With_feedback_DW.HILReadEncoderTimebase_Task,
        (t_clock) p4p4With_feedback_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
